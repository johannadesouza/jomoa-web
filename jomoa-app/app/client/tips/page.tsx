...
export default function ClientTips() {
  ...
  const [selectedPhase, setSelectedPhase] = useState<CyclePhase | "all" | "general">("all");
  ...
  // If "general" is selected, we need to fetch all tips and filter for phase = null client-side
  const phaseFilter: CyclePhase | null = selectedPhase === "all" || selectedPhase === "general" ? null : selectedPhase;
  const categoryFilter: Tip["category"] | null = selectedCategory === "all" ? null : selectedCategory;
  const { tips: allTips, loading, error } = useTips(phaseFilter, categoryFilter);
  
  // Filter for general tips (phase = null) if "general" is selected
  const tips = selectedPhase === "general" 
    ? allTips.filter(tip => tip.phase === null)
    : allTips;
            ...
            <div>
              ...
              <Select
                ...
                onChange={(e) => {
                  ...
                  if (value === "all" || value === "menstruation" || value === "follicular" || value === "ovulation" || value === "luteal" || value === "general") {
                    setSelectedPhase(value as CyclePhase | "all" | "general");
                  }
                }}
              >
                ...
                <option value="general">Allmänna tips</option>
              </Select>
            </div>
            <div>
