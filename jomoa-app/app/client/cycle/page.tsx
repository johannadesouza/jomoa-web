...
export default function ClientCyclePage() {
  ...
  const handleLogPeriodStart = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Mensstart loggad!");
      setPeriodStartDate("");
      await fetchLatestPeriodStart();
      await fetchRecentPeriodStarts();
    } catch (err) {
      console.error("Error logging period start:", err);
      const errorMessage = getErrorMessage(err) || "Kunde inte logga mensstart. Försök igen senare.";
      toast.error("Kunde inte logga mensstart", errorMessage);
    } finally {
      setSaving(false);
    }
  };
