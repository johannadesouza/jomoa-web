...
export default function CoachClients() {
  ...
  const handleCreateClient = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Klient skapad!", "Glöm inte att skicka lösenordet till klienten.");
      
      // Uppdatera klientlistan
      await fetchClients();
    } catch (err) {
      console.error("Error creating client:", err);
      const errorMessage = getUserFriendlyErrorMessage(err, ERROR_MESSAGES.CREATE_FAILED);
      toast.error("Kunde inte skapa klient", errorMessage);
    } finally {
  ...
  const handleSendInvite = async (e: React.FormEvent) => {
    ...
    if (!inviteEmail.trim()) {
      toast.error("Email krävs", ERROR_MESSAGES.REQUIRED_FIELD);
      return;
    }

    if (!user?.id) {
      toast.error("Du måste vara inloggad", "Du måste vara inloggad för att skicka inbjudan.");
      return;
    }
    ...
    if (!emailRegex.test(inviteEmail.trim())) {
      toast.error("Ogiltig email", ERROR_MESSAGES.INVALID_EMAIL);
      return;
    }
    ...
    try {
      ...
      setInviteLink(link);
      toast.success("Inbjudan skapad!", "Kopiera länken och skicka den till klienten.");
      setInviteEmail("");
    } catch (err) {
      console.error("Error sending invite:", err);
      const errorMessage = getUserFriendlyErrorMessage(err, ERROR_MESSAGES.INVITE_FAILED);
      toast.error("Kunde inte skapa inbjudan", errorMessage);
    } finally {
