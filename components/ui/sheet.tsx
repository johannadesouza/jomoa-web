...
>(({ side = "right", className, children, ...props }, ref) => {
  const sideClasses = {
    top: "inset-x-0 top-0 border-b rounded-b-[24px]",
    right: "inset-y-0 right-0 h-full w-3/4 border-l rounded-l-[24px] sm:max-w-sm",
    bottom: "inset-x-0 bottom-0 border-t rounded-t-[24px]",
    left: "inset-y-0 left-0 h-full w-3/4 border-r rounded-r-[24px] sm:max-w-sm",
  };
  ...
  return (
    <SheetPortal>
      ...
      <DialogPrimitive.Content
        ref={ref}
        className={cn(
          "fixed z-50 gap-4 bg-[#FEFCF8] p-6 shadow-[0_8px_32px_rgba(0,0,0,0.12)] transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
          sideClasses[side],
